...
// ZMIANA: klucz dla motywu (jasny/ciemny) i sortowania
const KLUCZ_MOTYWU    = "ectscalc_motyw";
const KLUCZ_SORTOWANIA = "ectscalc_sort";
...
let trybSredniej     = localStorage.getItem(KLUCZ_TRYBU) || "wazona";
...
// ZMIANA: stan sortowania — domyślnie kolejność dodania
let aktywneSort      = localStorage.getItem(KLUCZ_SORTOWANIA) || "domyslne";

// ---------- ZMIANA: sanityzacja tekstu (ochrona przed XSS) ----------

function sanityzuj(str) {
...
// ZMIANA: walidacja pojedynczego przedmiotu importowanego z zewnątrz
function walidujPrzedmiot(p) {
  if (!p || typeof p !== "object") return null;
  const nazwa = typeof p.nazwa === "string" ? p.nazwa.trim().slice(0, 120) : "";
  const ocena = parseFloat(p.ocena);
  const ects  = parseFloat(p.ects);
  const semRaw = p.semestr ?? p.sem ?? null;
  const semestr = parsujSemestr(String(semRaw ?? ""));
  if (!nazwa)                          return null;
  if (!DOSTEPNE_OCENY.includes(ocena)) return null;
  if (isNaN(ects) || ects < 0.5 || ects > 30) return null;
  return { identyfikator: Date.now() + Math.random(), nazwa, ocena, ects, semestr };
}

// ---------- helpers ----------

// ZMIANA: ujednolicono — jedyna funkcja do pobierania id, wszędzie używa `identyfikator`
const idPrzedmiotu    = (p) => p.identyfikator;
...
// ---------- ZMIANA: sortowanie listy przedmiotów ----------

function sortujPrzedmioty(lista) {
  const kopia = [...lista];
  switch (aktywneSort) {
    case "nazwa-az":  return kopia.sort((a, b) => a.nazwa.localeCompare(b.nazwa, "pl"));
    case "nazwa-za":  return kopia.sort((a, b) => b.nazwa.localeCompare(a.nazwa, "pl"));
    case "ocena-desc": return kopia.sort((a, b) => b.ocena - a.ocena || a.nazwa.localeCompare(b.nazwa, "pl"));
    case "ocena-asc":  return kopia.sort((a, b) => a.ocena - b.ocena || a.nazwa.localeCompare(b.nazwa, "pl"));
    case "ects-desc":  return kopia.sort((a, b) => b.ects - a.ects || a.nazwa.localeCompare(b.nazwa, "pl"));
    case "ects-asc":   return kopia.sort((a, b) => a.ects - b.ects || a.nazwa.localeCompare(b.nazwa, "pl"));
    default:           return kopia;
  }
}
...
// ---------- ZMIANA: motyw jasny / ciemny ----------

function ustawMotyw(motyw, zapiszLS = true) {
  document.documentElement.setAttribute("data-motyw", motyw);
  if (zapiszLS) localStorage.setItem(KLUCZ_MOTYWU, motyw);
  const btn = $("#btn-motyw");
  if (btn) {
    btn.title = motyw === "noc" ? "Przełącz na tryb jasny" : "Przełącz na tryb ciemny";
    btn.setAttribute("aria-label", btn.title);
    btn.querySelector(".motyw-ikona-noc").style.display  = motyw === "noc"  ? "none" : "";
    btn.querySelector(".motyw-ikona-dzien").style.display = motyw === "dzien" ? "none" : "";
  }
}
...
function obliczCelSredniej(srednia, sumaEcts, pozwolKonfetti = false) {
  ...
  if (srednia >= cel) {
    ...
    // ZMIANA: zabezpieczenie przed dzieleniem przez zero gdy cel = 5.0
    const mianownik = 5 - cel;
    let potrzebaEcts;
    let komunikatEcts;
    if (mianownik < 0.001) {
      potrzebaEcts = null;
      komunikatEcts = "Cel 5,0 jest osiągalny wyłącznie przy wszystkich ocenach 5,0.";
    } else if (trybSredniej === "wazona") {
      potrzebaEcts = Math.max(0, (cel * sumaEcts - sw) / mianownik);
      komunikatEcts = `Potrzebujesz ok. <strong>${potrzebaEcts.toFixed(1)} ECTS</strong> z oceną 5,0.`;
    } else {
      const sumOcen = przedmioty.reduce((s, p) => s + p.ocena, 0);
      potrzebaEcts = Math.max(0, (cel * (przedmioty.length + 1) - sumOcen) / mianownik);
      komunikatEcts = `Potrzebujesz ok. <strong>${Math.ceil(potrzebaEcts)}</strong> przedmiotu(-ów) z oceną 5,0.`;
    }
    e.celInfo.innerHTML = `Brakuje <strong>${Math.abs(roznica).toFixed(3)} pkt</strong>. ${komunikatEcts}`;
...
function odswiezStatystyki() {
  ...
  const btnCsv = document.getElementById("btn-eksport-csv");
  if (btnCsv) btnCsv.classList.toggle("ukryty", !liczba);
...
function renderujWykresLinii() {
  ...
  if (klucze.length < 2) { kontener.style.display = "none"; return; }
  ...
  let tooltips = "", pointsHtml = "", labelHtml = "";
...
// ---------- ZMIANA: kontrolka sortowania ----------

function renderujKontrolkeSortowania() {
  const istniejaca = $("#sort-kontrolka");
  if (istniejaca) istniejaca.remove();

  const listaGora = $(".lista-gora");
  if (!listaGora || !przedmioty.length) return;

  const opcje = [
    { val: "domyslne",   label: "Kolejność dodania" },
    { val: "nazwa-az",   label: "Nazwa A–Z" },
    { val: "nazwa-za",   label: "Nazwa Z–A" },
    { val: "ocena-desc", label: "Ocena: malejąco" },
    { val: "ocena-asc",  label: "Ocena: rosnąco" },
    { val: "ects-desc",  label: "ECTS: malejąco" },
    { val: "ects-asc",   label: "ECTS: rosnąco" },
  ];

  const wrapper = document.createElement("div");
  wrapper.id = "sort-kontrolka";
  wrapper.style.cssText = "display:flex;align-items:center;gap:6px;margin-bottom:8px;";

  const label = document.createElement("span");
  label.textContent = "Sortuj:";
  label.style.cssText = "font-size:12px;color:var(--tekst-mid);white-space:nowrap;";

  const select = document.createElement("select");
  select.style.cssText = "font-size:12px;padding:3px 7px;border-radius:8px;background:var(--tlo-wejscie);color:var(--tekst);border:1px solid var(--ramka-mid);cursor:pointer;";
  select.setAttribute("aria-label", "Sortowanie listy przedmiotów");

  opcje.forEach(o => {
    const opt = document.createElement("option");
    opt.value = o.val;
    opt.textContent = o.label;
    if (o.val === aktywneSort) opt.selected = true;
    select.appendChild(opt);
  });

  select.addEventListener("change", () => {
    aktywneSort = select.value;
    localStorage.setItem(KLUCZ_SORTOWANIA, aktywneSort);
    renderujListe();
  });
  ...
  // Wstaw po .lista-gora
  listaGora.insertAdjacentElement("afterend", wrapper);
}
...
function renderujListe() {
  ...
  widoczne.forEach((sem) => {
    // ZMIANA: sortowanie stosowane wewnątrz każdej grupy semestru
    const listaGrupy = sortujPrzedmioty(grupy[sem]);
    ...
    listaGrupy.forEach((p, i) => {
      const li = document.createElement("li");
      li.className = "element-listy " + klasaOceny(p.ocena);
      li.style.animationDelay = `${i * 30}ms`;
      // ZMIANA: sanityzacja nazwy przy renderowaniu (ochrona XSS)
      li.innerHTML = `
        <span class="element-ocena">${p.ocena.toFixed(1)}</span>
        <div class="element-info">
          <div class="element-nazwa" title="${sanityzuj(p.nazwa)}">${sanityzuj(p.nazwa)}</div>
...
function obliczCoJesli() {
  ...
  const noweEkstra = wierszeCoJesli
    .filter(w => w.ects > 0)
    .map(w => ({ ocena: w.ocena, ects: w.ects, semestr: null, identyfikator: Math.random() }));
  const potem = obliczSrednia([...przedmioty, ...noweEkstra]);
...
function obliczStypendium() {
  ...
  // ZMIANA: profesjonalny ton komunikatów stypendialnych
  ...
  if (srednia >= prog) {
    ...
    if (strefaRyzyka) {
      // ZMIANA: zabezpieczenie przed mianownikiem = 0
      const mianownikStyp = 5 - prog;
      const potrzebaEcts = mianownikStyp > 0.001 ? Math.max(0, (prog * sumaEcts - sw) / mianownikStyp) : null;
      const etsInfo = potrzebaEcts !== null ? ` Ok. ${Math.ceil(potrzebaEcts)} ECTS z oceną 5,0 buduje bezpieczny zapas.` : "";
      e.komunikatStypendium.innerHTML = `<strong>Strefa ryzyka.</strong> Twoja średnia przekracza próg zaledwie o ${roznica.toFixed(3)} pkt — jedno słabsze zaliczenie może zmienić wynik.${etsInfo}`;
    } else {
      e.komunikatStypendium.innerHTML = `<strong>Powyżej progu.</strong> Zapas: ${roznica.toFixed(3)} pkt. Szacunkowo ${przed} ${przed === 1 ? "osoba ma" : "osoby mają"} wyższą średnią.`;
    }
  } else {
    ...
    const mianownikStyp = 5 - prog;
    const potrzebaEcts = mianownikStyp > 0.001 ? Math.max(0, (prog * sumaEcts - sw) / mianownikStyp) : null;
    const etsInfo = potrzebaEcts !== null ? ` Do progu ${prog.toFixed(2)} potrzebujesz ok. <em>${potrzebaEcts.toFixed(1)} ECTS</em> z oceną 5,0.` : "";
    if (strefaRyzyka) {
      e.komunikatStypendium.innerHTML = `<strong>Strefa ryzyka.</strong> Brakuje zaledwie ${Math.abs(roznica).toFixed(3)} pkt.${etsInfo}`;
    } else {
      e.komunikatStypendium.innerHTML = `<strong>Brakuje: ${Math.abs(roznica).toFixed(3)} pkt.</strong> Szacunkowo ${przed} ${przed === 1 ? "osoba jest" : "osoby są"} przed Tobą.${etsInfo}`;
    }
  }
}
...
function eksportujDane() {
  ...
  try {
    ...
    pokazPowiadomienie("Błąd podczas eksportu danych.", "blad");
  }
}

// ZMIANA: eksport do CSV
function eksportujCSV() {
  if (!przedmioty.length) {
    pokazPowiadomienie("Brak przedmiotów do eksportu.", "blad");
    return;
  }
  try {
    const naglowek = "nazwa,ocena,ects,semestr";
    const wiersze = przedmioty.map(p => {
      const nazwa = `"${p.nazwa.replace(/"/g, '""')}"`;
      return `${nazwa},${p.ocena},${p.ects},${p.semestr || ""}`;
    });
    ...
    a.href     = url;
    a.download = "ectscalc-eksport.csv";
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 10000);
    pokazPowiadomienie("Plik CSV został pobrany.");
  } catch (err) {
    pokazPowiadomienie("Błąd podczas eksportu do CSV.", "blad");
  }
}

// ZMIANA: import z pliku CSV z auto-detekcją kolumn
function importujCSV(plik) {
  if (!plik) return;
  const reader = new FileReader();
  reader.onload = (ev) => {
    try {
      const tekst = ev.target.result.replace(/^\uFEFF/, ""); // usuń BOM
      const linie = tekst.split(/\r?\n/).filter(l => l.trim());
      if (linie.length < 2) { pokazPowiadomienie("Plik CSV jest pusty lub nieprawidłowy.", "blad"); return; }

      // Auto-detekcja separatora
      const separator = linie[0].includes(";") ? ";" : ",";

      // Parsowanie nagłówka
      const naglowek = parsujLinieCSV(linie[0], separator).map(h => h.toLowerCase().trim());
      const idxNazwa  = naglowek.findIndex(h => ["nazwa", "name", "przedmiot", "subject"].includes(h));
      const idxOcena  = naglowek.findIndex(h => ["ocena", "grade", "ocena końcowa"].includes(h));
      const idxEcts   = naglowek.findIndex(h => ["ects", "punkty ects", "credits"].includes(h));
      const idxSem    = naglowek.findIndex(h => ["semestr", "sem", "semester"].includes(h));

      if (idxNazwa === -1 || idxOcena === -1 || idxEcts === -1) {
        pokazPowiadomienie("Nie odnaleziono kolumn: nazwa, ocena, ects.", "blad");
        return;
      }

      const zaimportowane = [];
      for (let i = 1; i < linie.length; i++) {
        const komorki = parsujLinieCSV(linie[i], separator);
        const surowe = {
          nazwa:   komorki[idxNazwa] || "",
          ocena:   komorki[idxOcena],
          ects:    komorki[idxEcts],
          semestr: idxSem >= 0 ? komorki[idxSem] : null,
        };
        const p = walidujPrzedmiot(surowe);
        if (p) {
          // Nadaj unikalny identyfikator
          p.identyfikator = Date.now() + i;
          zaimportowane.push(p);
        }
      }

      if (!zaimportowane.length) {
        pokazPowiadomienie("Nie znaleziono prawidłowych wierszy w pliku CSV.", "blad");
        return;
      }

      pendingImportData = zaimportowane;
      const modal = document.getElementById("modal-import");
      const opis  = modal && modal.querySelector(".modal-import-opis");
      if (opis) opis.textContent = `Znaleziono ${zaimportowane.length} przedmiotów w pliku CSV. Wczytać je? Twoje obecne dane zostaną zastąpione.`;
      if (modal) modal.classList.remove("ukryty");
    } catch (err) {
      pokazPowiadomienie("Błąd podczas przetwarzania pliku CSV.", "blad");
    }
  };
  reader.onerror = () => pokazPowiadomienie("Nie udało się odczytać pliku.", "blad");
  reader.readAsText(plik, "UTF-8");
}

// ZMIANA: parser CSV obsługujący pola w cudzysłowach
function parsujLinieCSV(linia, sep = ",") {
  const wynik = [];
  let aktualne = "", wCudzSlowie = false;
  for (let i = 0; i < linia.length; i++) {
    const c = linia[i];
    if (c === '"') {
      if (wCudzSlowie && linia[i + 1] === '"') { aktualne += '"'; i++; }
      else wCudzSlowie = !wCudzSlowie;
    } else if (c === sep && !wCudzSlowie) {
      wynik.push(aktualne.trim());
      aktualne = "";
    } else {
      aktualne += c;
    }
...
function sprawdzHash() {
  ...
  try {
    ...
    // ZMIANA: walidacja i sanityzacja każdego przedmiotu z URL
    const bezpieczne = dane
      .map(p => walidujPrzedmiot(p))
      .filter(Boolean)
      .map((p, i) => ({ ...p, identyfikator: Date.now() + i }));
    if (!bezpieczne.length) return;
    pendingImportData = bezpieczne;
...
function zapiszEdycje() {
  ...
  // ZMIANA: profesjonalne komunikaty walidacyjne
  if (!nazwa)             { e.modalNazwa.focus(); pokazPowiadomienie("Nazwa przedmiotu jest wymagana.", "blad"); return; }
  if (!ocena)             { pokazPowiadomienie("Wybierz ocenę.", "blad"); return; }
  if (!ects || ects <= 0) { pokazPowiadomienie("Podaj prawidłową liczbę punktów ECTS.", "blad"); return; }
...
if (e.segmentedWazona) e.segmentedWazona.addEventListener("click", () => ustawTrybSredniej("wazona"));
if (e.segmentedArytmetyczna) e.segmentedArytmetyczna.addEventListener("click", () => ustawTrybSredniej("arytmetyczna"));
...
e.formularz.addEventListener("submit", (ev) => {
  ...
  // ZMIANA: dodana walidacja pustej nazwy
  if (!nazwa)             { pokazPowiadomienie("Nazwa przedmiotu jest wymagana.", "blad"); e.poleNazwy.focus(); return; }
  if (!ocena)             { pokazPowiadomienie("Wybierz ocenę.", "blad"); e.poleOceny.focus(); return; }
  if (!ects || ects <= 0) { pokazPowiadomienie("Podaj prawidłową liczbę punktów ECTS.", "blad"); e.poleEcts.focus(); return; }
  // ZMIANA: ujednolicono identyfikator — tylko `identyfikator`, nigdy `id`
...
e.przyciskWyczyszczenia.addEventListener("click", () => {
  ...
  zapiszDane();
  renderujWszystko();
  pokazPowiadomienie("Wyczyszczono listę przedmiotów.", "ok", {
    etykieta: "Cofnij",
...
// ZMIANA: przełącznik motywu — nasłuchiwanie na przycisk wstrzyknięty przez HTML
document.addEventListener("click", (ev) => {
  const btn = ev.target.closest("#btn-motyw");
  if (!btn) return;
  const obecny = document.documentElement.getAttribute("data-motyw") || "noc";
  ustawMotyw(obecny === "noc" ? "dzien" : "noc");
});

// ZMIANA: przycisk eksportu CSV — delegacja zdarzeń
document.addEventListener("click", (ev) => {
  if (ev.target.closest("#btn-eksport-csv")) eksportujCSV();
});

// ZMIANA: obsługa importu CSV przez input[type=file]
document.addEventListener("change", (ev) => {
  const input = ev.target.closest("#input-csv");
  if (!input || !input.files || !input.files[0]) return;
  importujCSV(input.files[0]);
  input.value = ""; // reset żeby można było wczytać ten sam plik ponownie
});

// ZMIANA: drag & drop CSV na obszar listy
const kartaListy = document.getElementById("karta-lista");
if (kartaListy) {
  kartaListy.addEventListener("dragover", (ev) => {
    ev.preventDefault();
    kartaListy.classList.add("drag-over");
  });
  kartaListy.addEventListener("dragleave", () => kartaListy.classList.remove("drag-over"));
  kartaListy.addEventListener("drop", (ev) => {
    ev.preventDefault();
    kartaListy.classList.remove("drag-over");
    const plik = ev.dataTransfer.files[0];
    if (plik && (plik.type === "text/csv" || plik.name.endsWith(".csv"))) {
      importujCSV(plik);
    } else {
      pokazPowiadomienie("Obsługiwany jest wyłącznie format CSV.", "blad");
    }
  });
}
...
// ZMIANA: inicjalizacja motywu z localStorage
const zapisanyMotyw = localStorage.getItem(KLUCZ_MOTYWU) || "noc";
ustawMotyw(zapisanyMotyw, false);
